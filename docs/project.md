...
| Storage | Browser `localStorage` (keys `_deviceId2`, `connected_servers`, `michelly_sessions`, `michelly_default_user`, `michelly_playlists_v2`, legacy read-only `michelly_playlists`) |
...
| `michelly_playlists_v2` | The `localStorage` map of per-server **dynamic named playlists** `{playlists: [{id, name, items:[entry…]}]}`, keyed by server id (like `michelly_sessions`); entry = `{Id, Type, Name, ImageTags?, Disabled?}`, Audio only. |
| `michelly_playlists` | The **legacy, read-only** per-server 3-slot store `{slots: [[entry…], [], []]}` (three preset slots), keyed by server id; never written or deleted since Tier 3, migrated losslessly/lazily into `michelly_playlists_v2`. |
...
| `window.Michelly` | The app's shared JS namespace (`platform`, `api`, `auth`, `ui`, `catalog`, `music` (artist-centric music browse with Genres/Albums/Songs dimensions, shelves and functional search), `player`, `audio`, `playlist` (ordered-list session with playback modes + queue), `playlists` (dynamic named-playlist store, with the legacy index-based aliases)) built by `frontend/js/app/*`. |
...
- [`context/aimp-design-reference.md`](context/aimp-design-reference.md) — design-reference mining
  ...
  **Tier 1 (playback modes + queue, item-actions menu) implemented and committed (HEAD `0e86bd7`);
  Tiers 2–3 (browse dimensions/search, dynamic playlists) implemented but uncommitted and pending
