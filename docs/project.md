...
| `window.Michelly` | The app's shared JS namespace (`platform`, `api`, `auth`, `ui`, `catalog`, `music` (artist-centric music browse), `player`, `audio`, `playlist` (ordered-list session), `playlists` (saved-playlist store)) built by `frontend/js/app/*`. |
